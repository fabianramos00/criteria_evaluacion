...
from src.constants import METADATA_DATE_REGEX, DATE_FORMATS, ACCESS_STANDARD_VALUES, RESULT_TYPES, FORMAT_DICT, \
...
def check_metadata_date(page_parse: BeautifulSoup) -> tuple[dict[str, str] | None, bool | None]:
    date_dict = {}
...
def check_format(format_list: list[str] | None) -> str | None:
    ...
    for fmt in format_list:
        parts = fmt.split("/")
        if len(parts) == 2 and parts[0] in FORMAT_DICT and parts[1] in FORMAT_DICT[parts[0]]:
            return fmt
    return None
...
def check_author_id(author_values: list[str] | None) -> str | None:
    ...
    if len(author_values) > 1:
        if all(IRALIS_BASE_URL_ID in v or ORCID_BASE_URL_ID in v for v in author_values):
            return 'ORCID/IraLIS'
        return None
    v = author_values[0]
...
async def get_metadata(link_dict: dict) -> dict | None:
    ...
    url_dict['single_type_research_result'] = (
        url_dict['standard_type_research_result'][0] 
        if url_dict['standard_type_research_result'] is not None and 
        len(url_dict['standard_type_research_result']) == 1 else None
    )
    url_dict['single_version'] = (
        url_dict['standard_version_coar'][0] 
        if url_dict['standard_version_coar'] is not None and 
        len(url_dict['standard_version_coar']) == 1 else None
    )
...
def validate_metadata(url_dict_list: list[dict]) -> tuple[dict, dict]:
    fields_metadata_dict = {f: {'value': True, 'details': []} for f in METADATA_FIELDS + ['DC.date']}
...
async def evaluate_metadata(
    ...
    new_link_list = await asyncio.gather(*(limited_get_metadata(link) for link in link_list))
    result_metadata, result_fields = validate_metadata(new_link_list)
    metadata_resume['first_fields'] = evaluate_metadata_group(
        result_metadata,
        ['DC.creator', 'DC.title', 'DC.type', 'DC.date', 'DC.rights']
    )
    metadata_resume['second_fields'] = evaluate_metadata_group(
        result_metadata,
        [
            'DC.description', 'DC.format', 'DC.language', 'DC.identifier', 
            'DC.subject', 'DC.contributor', 'DC.relation', 'DC.publisher'
        ]
    )
