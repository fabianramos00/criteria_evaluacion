...
async def get_open_aire_links(data_source_id: str, repository_url: str) -> list:
    ...
    async with httpx.AsyncClient() as client:
        params: dict = {
            ...
            'page': 1,
            'pageSize': 10,
            'fromPublicationDate': (date.today() - relativedelta(years=settings.MIN_YEAR_DIFFERENCE)).strftime('%Y')
        }
        ...
        for datasource in data_oa['results']:
            if  is_similar(datasource["officialName"], repository_name):
                return {
                    ...
                    'links': await get_open_aire_links(datasource['id'], datasource.get('websiteUrl'))
                }
    return None


async def get_open_alex_data(repository_url: str) -> dict | None:
    repository_url = repository_url.replace('https://', '').replace('http://', '').rstrip('/')
    ...
    async with httpx.AsyncClient() as client:
        ...
        for data_provider in core_result['results']:
            ...
            if is_similar(data_provider.get('name'), repository_name):
                return {'name': data_provider.get('name'), 'host': data_provider.get('homepageUrl')}
    return None
...
def friendly_secure_url(repository_url: str) -> dict:
    ...
    if len(host_clear) > settings.FRIENDLY_URL_LENGTH or not re.match(r'^[a-z0-9.-/]+$', host_clear):
...
def count_national_collectors(collector_data: list[str] | None):
    ...
    if 5 == len(collector_data):
        return {'value': 1.5, 'text': 'Presence in 5 national collectors', 'details': collector_data}
    return {'value': 1, 'text': 'Presence in 1 or more national collectors', 'details': collector_data}
...
async def execute_async_search(func_dict: dict, repository_name_list: list):
    tasks = (
        search_in(func_dict[key], repository_name_list)
...
async def evaluate_visibility(
    ...
    resume_visibility = {
        ...
        'national_collector': count_national_collectors(visibility_schema.collector_urls),
        'url': friendly_secure_url(record.repository_url)
    }
    data_links: dict = resume_visibility['directory']['data'].copy() | resume_visibility['collector']['data'].copy()
    resume_visibility['standard'] = standard_name(data_links)
    resume_visibility['directory'].update(count_items(resume_visibility['directory']['data'], 'national directories'))
    resume_visibility['collector'].update(count_items(resume_visibility['collector']['data'], 'international collectors'))
