...
from pydantic import BaseModel, AnyHttpUrl, field_validator, Field, computed_field
...
class VisibilitySchema(BaseModel):
    ...
    _validate_urls = field_validator('collector_url1', 'collector_url2', 'collector_url3',
                                      'collector_url4', 'collector_url5')(validate_website_url)
    _validate_required = field_validator('collector_url1')(conditional_required('national_collector'))
...
class PolicySchema(BaseModel):
    ...
    _validate_urls = field_validator('open_access_url', 'metadata_reuse_url', 'content_preservation_url',
                                      'deposit_data_url', 'action_policy_url', 'policy_data_url',
                                      'vision_mission_url', 'contact_url')(validate_website_url)
    
    _validate_open_access = field_validator('open_access_url')(conditional_required('open_access'))
    _validate_metadata_reuse = field_validator('metadata_reuse_url')(conditional_required('metadata_reuse'))
    _validate_content_preservation = field_validator('content_preservation_url')(conditional_required('content_preservation'))
    _validate_deposit_data = field_validator('deposit_data_url')(conditional_required('deposit_data'))
...
class LegalAspectsSchema(BaseModel):
    ...
    _validate_required = field_validator('author_permission_url')(conditional_required('author_permission'))
...
class SecuritySchema(BaseModel):
    ...
    _validate_urls = field_validator('backups_url', 'checksum_url')(validate_website_url)
...
class StatisticsSchema(BaseModel):
    ...
    _validate_required = field_validator('general_statistics_url')(
        combined_validator(conditional_required('general_statistics'), url_must_contain())
    )
...
class ServicesSchema(BaseModel):
    ...
    _validate_urls = field_validator('author_profiles_url', 'cite_metrics_url', 
                                      'new_metrics_url')(validate_website_url)
